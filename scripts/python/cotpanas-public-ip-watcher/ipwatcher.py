    ...
    if Path(filepath).is_file():
        ...
        for line in lines:
            ...
            if (line[:1] != "#" and line.strip()):
                ...
                elif (param == "save_log_path"):
                    configObj.save_log_path = value
                elif (param == "named_location"):
                    configObj.named_location = value
...
def trigger_azure_function(filepath, ip_address, named_location):
    ...
    # Write the response from the Azure Function to a log file
    with open(filepath, 'w') as f:
        f.write(response.text + '\n')

    # Print the response from the Azure Function
    print(response.text)
...
if (len(sys.argv) != 2):
    ...
    if ((curr_external_ip != old_external_ip) or (curr_local_ip != old_local_ip)):
        ...
        # Trigger the Azure Function with curr_external_ip
        trigger_azure_function(config.save_log_path,curr_external_ip,config.named_location)
