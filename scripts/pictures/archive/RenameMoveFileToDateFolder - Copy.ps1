...
$picMetadata = Get-FileMetaData -folder (Get-ChildItem $SourceDir -Recurse -Directory -Force).FullName
...
foreach ($Img in $ImgFiles) 
{
    ...
    $dateTaken = [datetime]::ParseExact($dateString,"yyyy:MM:dd HH:mm:ss`0",$Null).ToString('yyyyMMdd_HHmmss')
